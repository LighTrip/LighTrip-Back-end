...
ALTER TABLE public.district_cover ADD COLUMN IF NOT EXISTS team_id bigint;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_district_cover_team'
    ) THEN
...
ALTER TABLE public.district_cover DROP CONSTRAINT IF EXISTS uk_district_cover_user_district;

CREATE UNIQUE INDEX IF NOT EXISTS uk_district_cover_user_district
    ON public.district_cover (user_id, district_category)
    WHERE team_id IS NULL;

CREATE UNIQUE INDEX IF NOT EXISTS uk_district_cover_team_district
